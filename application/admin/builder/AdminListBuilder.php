...
class AdminListBuilder extends AdminBuilder
{
    ...
    public function addBtn($position='head',$type, $attribute = null)
    {
        switch ($type) {
            ...
            case 'all':  // 添加全选按钮
                // 预定义按钮属性以简化使用
                $my_attribute['title'] = '全选';
                $my_attribute['class'] = 'all';
            ...
            case 'restore':  // 添加还原按钮(回收的反操作)
                ...
                break;
            case 'delete': 
                // 添加删除按钮(我没有反操作，删除了就没有了，就真的找不回来了)
                // 预定义按钮属性以简化使用
                $my_attribute['title']     = '批量删除';
                $my_attribute['href']      = 'javascript:;';
                $my_attribute['data-ids']  = '';
                $my_attribute['data-type'] = 'all';
                $my_attribute['class']     = 'isAllowed';
                $my_attribute['id']        = 'btnDelete';
                $my_attribute['model']     = $attribute['model'] ? : CONTROLLER_NAME;
                $my_attribute['data-url']  = url(
                    MODULE_NAME.'/'.CONTROLLER_NAME.'/setStatus',
                    array(
                        'status' => 'delete',
                        'model' => $my_attribute['model']
    ...
    public function addRightButton($type, $attribute = null)
    {
        switch ($type) {
            case 'edit':  // 编辑按钮
                ...
                $my_attribute['class']     = ' btn-xs btn-link';
                $my_attribute['href']  = url(
            ...
            case 'forbid':  // 改变记录状态按钮，会更具数据当前的状态自动选择应该显示启用/禁用
                ...
                $my_attribute['0']['class'] = 'check_a btn-xs btn-link isConfirm';
                $my_attribute['0']['data-url']  = url(
                    MODULE_NAME.'/'.CONTROLLER_NAME.'/setStatus',
                    array(
                        'status' => 'resume',
                        'ids' => '__data_id__',
                ...
                $my_attribute['1']['class'] = 'check_a btn-xs btn-link isConfirm';
                $my_attribute['1']['data-url']  = url(
                    MODULE_NAME.'/'.CONTROLLER_NAME.'/setStatus',
                    array(
                        'status' => 'forbid',
                        'ids' => '__data_id__',
            ...
            case 'hide':  // 改变记录状态按钮，会更具数据当前的状态自动选择应该显示隐藏/显示
                ...
                $my_attribute['2']['title'] = '隐藏';
                $my_attribute['2']['class'] = 'check_a btn-xs btn-link isConfirm';
                $my_attribute['2']['data-url']  = url(
                    MODULE_NAME.'/'.CONTROLLER_NAME.'/setStatus',
                    array(
                        'status' => 'hide',
                        'ids' => '__data_id__',
            ...
            case 'recycle':
                // 预定义按钮属性以简化使用
                $my_attribute['title'] = '回收';
                $my_attribute['class'] = 'check_a btn-xs btn-link isConfirm';
                $my_attribute['model'] = $attribute['model'] ? : CONTROLLER_NAME;
                $my_attribute['data-url'] = url(
                    MODULE_NAME.'/'.CONTROLLER_NAME.'/setStatus',
                    array(
            ...
            case 'delete':
                ...
                // $my_attribute['class'] = 'ajax-get confirm';
                // $my_attribute['model'] = $attribute['model'] ? : CONTROLLER_NAME;
                // $my_attribute['href'] = url(
                //     MODULE_NAME.'/'.CONTROLLER_NAME.'/setStatus',
                //     array(
                ...
                $my_attribute['title']     = '删除';
                $my_attribute['href']      = 'javascript:;';
                $my_attribute['data-ids']  = '__data_id__';
                $my_attribute['class']     = 'check_a btn-xs btn-link isConfirm';
                $my_attribute['model']     = $attribute['model'] ? : CONTROLLER_NAME;
                $my_attribute['data-url']  = url(
                    MODULE_NAME.'/'.CONTROLLER_NAME.'/setStatus',
    ...
    public function fetch($templateFile='',$vars ='', $replace ='', $config = '')
    {
        ...
        foreach ($this->_table_data_list as &$data) {
            ...
            if ($this->_right_button_list) {
                ...
                foreach ($this->_right_button_list as $right_button) {
                    ...
                    if (isset($right_button['data-ids'])) {
                        $right_button['data-ids'] = preg_replace(
                            '/__data_id__/i',
                            $data[$this->_table_data_list_key],
                            $right_button['data-ids']
                        );
                    }
                    if (isset($right_button['data-url'])) {
                        $right_button['data-url'] = preg_replace(
                            '/__data_id__/i',
                            $data[$this->_table_data_list_key],
                            $right_button['data-url']
                        );
                    }
                    if (isset($right_button['onclick'])) {
                        $right_button['onclick'] = preg_replace(
                            '/__data_id__/i',
                            $data[$this->_table_data_list_key],
                    ...
                    }elseif(isset($right_button['title'])){
            ...
            foreach ($this->_table_column_list as &$column) {
                switch ($column['type']) {
                    ...
                    case 'date':
                        ...
                        break;
                    case 'datetime':
                        $data[$column['name']] = time_format($data[$column['name']],'Y-m-d H:i:s');
                        break;
                    case 'time':
                    ...
                    case 'avatar':
                        $data[$column['name']] = '<img style="width:40px;height:40px;" src="'.cdn_img_url(root_full_url($data[$column['name']]),'',1).'">';
                        break;
                    case 'cover':
                        $data[$column['name']] = '<img class="cover" width="90" src="'.cdn_img_url($data[$column['name']],'',1).'">';
                        break;
                    case 'picture':
                        $data[$column['name']] = '<img class="picture" width="120" src="'.cdn_img_url(root_full_url($data[$column['name']]),'',1).'">';
                        break;
                    case 'pictures':
                        $temp = explode(',', $data[$column['name']]);
                        $data[$column['name']] = '<img class="pictures" width="120" src="'.root_full_url($temp[0]).'">';
                        break;
                    // case 'link':
                    //     $url_attribute='';
                    ...
                    case 'array'://新增
                        ...
                        if (is_array($column['param'])) {
                            ...
                            //dump($column_array[$data[$column['name']]]);
                        ...
                        // if (is_array($column['param'])) {
                        //     $data['type'] = $column['param'][$data['type']];
                        // }
                        break;
                    case 'callback': // 调用函数
                        if (is_array($column['param'])) {
        ...
        $this->assign('sub_title',          $this->_sub_title);          // 页面标题
